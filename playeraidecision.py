...
class PlayerMoveAIDecision:

    def __init__(self, game_state: gs.GameState):

        self.player_pokemon: p.BattlingPokemon = (
            game_state.player_info.team.active_pokemon
        )
    ...
    def get_highest_damage_move(self):
        moves_obj_unsanitised: list[m.Move | None] = list(
            self.player_pokemon.moveset.values()
        )
        ...
        print("\tSelecting player highest damage move")

        for move in moves_obj_list:

            damage: int = dc.DamageCalculation(
                self.player_pokemon, self.opponent_pokemon, move, final_calc=False
            ).final_damage

            move_damages.append(damage)

        print(f"\t\t{self.player_pokemon.pokemon_name.title()} damages: {move_damages}")

        highest_damaging_move_index = move_damages.index(max(move_damages))

        print(
            f"\t\t{self.player_pokemon.pokemon_name} highest damaging move is {moves_obj_list[highest_damaging_move_index].move_name}\n\n"
        )
